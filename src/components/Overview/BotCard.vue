    ...
    <v-card class="elevation-3 botCard" :to="`/bot/${value.id}`">
      <v-list-item two-line>
        <v-badge bottom offset-x="30" offset-y="32" class="progressBadge" :color="badgeType">
          ...
          <v-list-item-avatar class="ml-0" v-if="!value.info.unknown">
        ...
        <v-list-item-content>
          ...
          <v-list-item-subtitle>
            <span v-t="`botStatuses.${value.status}`"/>
        ...
        <v-list-item-action>
          <v-menu offset-y left>
            <template v-slot:activator="{ on }">
              <v-btn :disabled="blockCtl" color="primary" icon dark v-on="on" v-on:click.prevent>
                <v-icon>mdi-dots-vertical</v-icon>
              </v-btn>
            </template>
            <v-list dense>
              <v-list-item v-if="value.status === 'enabled'" @click="() => stateCtl('reload')" @click.stop="item.action">
                <v-list-item-icon><v-icon color="orange">mdi-refresh</v-icon></v-list-item-icon>
                <v-list-item-title v-t="'overview.cards.restart'"/>
              </v-list-item>
              <v-list-item v-if="value.status === 'enabled'" @click="() => stateCtl('disable')" @click.stop="item.action">
                <v-list-item-icon><v-icon color="red">mdi-power</v-icon></v-list-item-icon>
                <v-list-item-title v-t="'overview.cards.disable'"/>
              </v-list-item>
              <v-list-item v-else-if="allowStart" @click="() => stateCtl('enable')" @click.stop="item.action">
